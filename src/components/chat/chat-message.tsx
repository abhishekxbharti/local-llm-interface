...
import { Brain, RefreshCcw, Smile, User } from "lucide-react";
...
function ChatMessage({ message, isLast, isLoading, reload }: ChatMessageProps) {
  ...
  const renderActionButtons = () =>
    message.role === "assistant" && (
      <div className="pt-4 flex gap-4 items-center text-muted-foreground">
        {!isLoading && (
          <ButtonWithTooltip side="bottom" toolTipText="Copy">
            ...
            >
              {isCopied ? (
                <CheckIcon className="w-4 h-4 transition-all" />
              ) : (
                <CopyIcon className="w-4 h-4 transition-all" />
              )}
            </Button>
          </ButtonWithTooltip>
        )}
